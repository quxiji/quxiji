...
function tool_data_out_hang( data , lie1 , lie2){
  ...
  if(data_num.length >0 && parseFloat(data[3]) <100){ 
    ...
    document.getElementById("id_tool_"+data[0]).addEventListener("click", function(){ var data_id  = document.getElementById("id_tool_"+data[0]).value.split("\t"); var data_arr = [];  data_arr[0] = data_id[1].toString() + data_id[2].toString(); tool_copyToClip(data_arr); data_arr[0] = data_id[0]; tool_copyToClip(data_arr); }  );  
  }
  return data_out; 
}
